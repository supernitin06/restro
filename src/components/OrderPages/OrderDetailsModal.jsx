    ...
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        >
          ...
          {/* Header */}
          <div className="flex justify-between items-center px-6 py-4 bg-gradient-to-r from-blue-600 to-purple-600 dark:from-gray-800 dark:to-gray-900 text-white">
            <div>
              <h2 className="text-xl font-bold flex items-center gap-3">
                Order #{orderId}
                <span className="bg-white/20 dark:bg-white/10 backdrop-blur-sm px-3 py-1 rounded-full text-sm font-medium border border-white/30 dark:border-white/20">
                  {status}
                </span>
              </h2>
              <p className="text-blue-100 dark:text-gray-300 text-xs mt-1">
                {new Date(order.createdAt).toLocaleString()}
              </p>
            </div>
            <button
              onClick={onClose}
              className="p-2 bg-white/20 hover:bg-white/30 dark:bg-white/10 dark:hover:bg-white/20 backdrop-blur-sm rounded-full transition-all hover:scale-110"
            >
                    ...
                    return (
                      <div key={s} className="flex flex-col items-center relative z-10">
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold border-2 transition-all ${
                          isCurrent ? "bg-gradient-to-br from-blue-500 to-purple-500 border-white text-white shadow-lg shadow-blue-200" :
                          isCompleted ? "bg-green-500 border-green-500 text-white shadow-md" :
                          "bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 text-gray-500 dark:text-gray-400 shadow-sm"
                        }`}>
                        ...
                        <div className="mt-2 text-center">
                          ...
                          {stepInfo?.at && (
                            <span className="text-[10px] text-gray-500 dark:text-gray-400 mt-1 font-medium block">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
              ...
              <div className="lg:col-span-3 space-y-5">
                ...
                <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-semibold text-gray-900 dark:text-gray-100">{customer.name || "N/A"}</p>
                        <div className="flex items-center gap-1 mt-1">
                          <FiPhone className="w-3 h-3 text-gray-600 dark:text-gray-300" />
                          <p className="text-xs text-gray-600 dark:text-gray-300">{customer.phone || "N/A"}</p>
                        </div>
                        {customer.email && (
                          <div className="flex items-center gap-1 mt-0.5">
                            <FiMail className="w-3 h-3 text-gray-600 dark:text-gray-300" />
                            <p className="text-xs text-gray-600 dark:text-gray-300">{customer.email}</p>
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="pt-3 border-t border-gray-700/30">
                      <div className="flex items-start gap-2">
                        <FiMapPin className="w-4 h-4 text-blue-600 dark:text-blue-400 mt-0.5" />
                        <div>
                          <p className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Delivery Address</p>
                          <p className="text-sm text-gray-600 dark:text-gray-300">
                            {deliveryAddress.addressLine || "N/A"}, {deliveryAddress.city || "N/A"}
                ...
                <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
                  ...
                  {deliveryFlow.partner && deliveryFlow.partner._id ? (
                    <div className="space-y-3">
                      <div className="space-y-2">
                        <div className="flex justify-between items-center">
                          <span className="text-xs text-gray-500 dark:text-gray-400">Name</span>
                          <span className="text-sm font-semibold text-gray-900 dark:text-gray-100">{deliveryFlow.partner.name}</span>
                        </div>
                        <div className="flex justify-between items-center">
                          <span className="text-xs text-gray-500 dark:text-gray-400">Phone</span>
                          <span className="text-sm font-semibold text-gray-900 dark:text-gray-100">{deliveryFlow.partner.phone}</span>
                        </div>
                        {deliveryFlow.partner.vehicle && (
                          <div className="flex justify-between items-center">
                            <span className="text-xs text-gray-500 dark:text-gray-400">Vehicle</span>
                            <span className="text-sm font-semibold text-gray-900 dark:text-gray-100">{deliveryFlow.partner.vehicle}</span>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="pt-3 border-t border-gray-700/30 space-y-1">
                        <div className="flex justify-between items-center text-xs">
                          ...
                          <span className="font-medium text-gray-600 dark:text-gray-300">
                            {deliveryFlow.assignedAt ? new Date(deliveryFlow.assignedAt).toLocaleTimeString() : '-'}
                          </span>
                        </div>
                        <div className="flex justify-between items-center text-xs">
                          <span className="text-gray-500 dark:text-gray-400">Picked:</span>
                          <span className="font-medium text-gray-600 dark:text-gray-300">
                            {deliveryFlow.pickedAt ? new Date(deliveryFlow.pickedAt).toLocaleTimeString() : '-'}
                          </span>
                        </div>
                      </div>
                    </div>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 flex-1 flex flex-col overflow-hidden">
                  <div className="p-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/50 flex justify-between items-center">
                    ...
                    <span className="text-sm font-semibold text-blue-600 bg-blue-50 border-blue-100 dark:text-blue-300 dark:bg-blue-900/30 px-3 py-1 rounded-lg border dark:border-blue-800/50">
                      Total: ₹{price.itemsTotal || 0}
                    </span>
                  </div>

                  <div className="overflow-y-auto flex-1 p-3 space-y-2">
                    {items.map((item, i) => ( 
                      <div key={i} className="flex gap-3 p-3 hover:bg-gray-50/50 dark:hover:bg-gray-800/50 rounded-lg transition-colors border border-transparent hover:border-gray-200 dark:hover:border-gray-700">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex justify-between items-start">
                            <h4 className="text-sm font-semibold text-gray-900 dark:text-gray-100 truncate">{item.name || "Item"}</h4>
                          ...
                          {item.description && (
                            <p className="text-xs text-gray-600 dark:text-gray-300 mt-1 line-clamp-1">{item.description}</p>
                          )}
                          {item.addons?.length > 0 && (
                            <div className="flex gap-1.5 mt-2 flex-wrap">
                              {item.addons.map((add, idx) => (
                                <span key={idx} className="text-xs text-gray-600 dark:text-gray-300 bg-gray-100 border-gray-200 dark:bg-gray-700 dark:border-gray-600 px-2 py-1 rounded border">
                                  {add.name}
                                </span>
                              ))}
                            </div>
              ...
              <div className="lg:col-span-3 space-y-5">
                <div className="bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-900/30 dark:to-purple-900/30 rounded-xl shadow-sm border-blue-100 dark:border-blue-800/50 overflow-hidden">
                  <div className="p-4 bg-gradient-to-r from-blue-600 to-purple-600 dark:from-blue-900/50 dark:to-purple-900/50 text-white">
                    ...
                    <div className="flex justify-between items-end">
                      <div>
                        <p className="text-xs text-blue-200 uppercase tracking-wide">Grand Total</p>
                        <p className="text-2xl font-bold">₹{price.grandTotal || 0}</p>
                      </div>
                      <div className="text-right">
                        <span className={`inline-block px-3 py-1 rounded-full text-xs font-semibold ${
                          (payment.status === "PAID" || price.status === "PAID")
                            ? "bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300"
                            : "bg-orange-100 text-orange-800 dark:bg-orange-900/30 dark:text-orange-300"
                        }`}>
                          {payment.status || price.status || "PENDING"}
                        </span>
                        <p className="text-xs text-blue-200 mt-1.5">{payment.method || "COD"}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="p-4 bg-white dark:bg-gray-800 space-y-3">
                    <div className="flex justify-between items-center text-sm">
                      ...
                      <span className="font-semibold text-gray-900 dark:text-gray-100">₹{price.itemsTotal || 0}</span>
                    </div>
                    <div className="flex justify-between items-center text-sm">
                      <span className="text-gray-600 dark:text-gray-300">Tax</span>
                      <span className="font-semibold text-red-500">+ ₹{price.tax || 0}</span>
                    </div>
                    <div className="flex justify-between items-center text-sm">
                      <span className="text-gray-600 dark:text-gray-300">Delivery Fee</span>
                      <span className="font-semibold text-blue-500">+ ₹{price.deliveryFee || 0}</span>
                    </div>
                    {price.discount > 0 && (
                      <div className="flex justify-between items-center text-sm">
                        <span className="text-gray-600 dark:text-gray-300">Discount</span>
                        <span className="font-semibold text-green-500">- ₹{price.discount || 0}</span>
                      </div>
                    )}
                ...
                {/* Order Summary */}
                <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
                  <h3 className="text-sm font-semibold text-gray-600 dark:text-gray-300 mb-3">Order Summary</h3>
                  <div className="space-y-2">
                    <div className="flex justify-between items-center text-sm">
                    ...
                    <div className="flex justify-between items-center text-sm">
                      ...
                      <span className="font-medium text-blue-600 dark:text-blue-300">{status}</span>
                    </div>
                    <div className="flex justify-between items-center text-sm">
                      <span className="text-gray-600 dark:text-gray-300">Items</span>
                      <span className="font-medium text-gray-900 dark:text-gray-100">{items.length} items</span>
                    </div>
                    <div className="flex justify-between items-center text-sm">
                      <span className="text-gray-600 dark:text-gray-300">Order Time</span>
