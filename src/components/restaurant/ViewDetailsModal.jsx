...
import { X, MapPin, Calendar, Star, Building, Navigation, Mail, Phone, Globe, CheckCircle, AlertCircle } from "lucide-react";
...
const ViewDetailsModal = ({ restaurant, onClose, onApprove, onSuspend }) => {
          ...
          <div className="flex justify-between items-start">
            <div className="flex items-center gap-4">
              <div className="relative">
                ...
                <div className={`absolute -bottom-1 -right-1 w-6 h-6 rounded-full border-2 border-white dark:border-gray-800 flex items-center justify-center ${isActive ? 'bg-emerald-100 text-emerald-600' : 'bg-rose-100 text-rose-500'}`}>
                  {isActive ? <CheckCircle size={12} /> : <AlertCircle size={12} />}
                </div>
              </div>
              <div>
                <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100">{name}</h2>
                <p className="text-gray-600 dark:text-gray-400 font-medium">{brandName}</p>
                <div className="flex items-center gap-3 mt-2">
                  <span className={`px-3 py-1 rounded-full text-xs font-semibold ${isActive ? 'bg-emerald-50 text-emerald-700 border border-emerald-200 dark:bg-emerald-900/30 dark:text-emerald-400 dark:border-emerald-800' : 'bg-rose-50 text-rose-700 border border-rose-200 dark:bg-rose-900/30 dark:text-rose-400 dark:border-rose-800'}`}>
        ...
        <div className="p-5 space-y-5 bg-gray-50 dark:bg-gray-800/50">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            <div className="bg-gradient-to-br from-white to-blue-50 dark:from-gray-800 dark:to-gray-800/50 rounded-xl p-5 border border-blue-100 dark:border-gray-700 shadow-soft">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h3 className="font-semibold text-gray-900 dark:text-gray-100">Address Details</h3>
              </div>
              {restaurant.address ? (
                <div className="space-y-3">
                  {[
                    { label: "Street", value: restaurant.address.street, color: "text-gray-800 dark:text-gray-200" },
                    { label: "City", value: restaurant.address.city, color: "text-gray-800 dark:text-gray-200" },
                    { label: "State", value: restaurant.address.state, color: "text-gray-800 dark:text-gray-200" },
                    { label: "ZIP Code", value: restaurant.address.zipCode, color: "text-gray-800 dark:text-gray-200" },
                    { label: "Country", value: restaurant.address.country, color: "text-gray-800 dark:text-gray-200" }
                  ].map((item, index) => (
                ...
                <div className="text-center py-6">
                  ...
                  <p className="text-gray-400 dark:text-gray-500 font-medium">No address provided</p>
                </div>
              )}
            </div>
            ...
            <div className="bg-gradient-to-br from-white to-amber-50 dark:from-gray-800 dark:to-gray-800/50 rounded-xl p-5 border border-amber-100 dark:border-gray-700 shadow-soft">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h3 className="font-semibold text-gray-900 dark:text-gray-100">Customer Ratings</h3>
              </div>
              <div className="flex items-center justify-between">
                <div>
                  <div className="flex items-baseline gap-2">
                    <span className="text-3xl font-bold text-gray-900 dark:text-gray-100">{restaurant.ratings?.average || 0}</span>
                ...
                <div className="text-right">
                  ...
                  <div className="text-2xl font-bold text-gray-900 dark:text-gray-100">{restaurant.ratings?.totalReviews || 0}</div>
                  <div className="text-xs text-gray-400 dark:text-gray-500 mt-1">Verified Customers</div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-white to-purple-50 dark:from-gray-800 dark:to-gray-800/50 rounded-xl p-5 border border-purple-100 dark:border-gray-700 shadow-soft lg:col-span-2">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h3 className="font-semibold text-gray-900 dark:text-gray-100">Geographic Location</h3>
              </div>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-white dark:bg-gray-900/50 rounded-lg p-4 border border-purple-100 dark:border-gray-700 text-center">
                      <div className="text-2xl font-bold text-purple-600 dark:text-purple-400">{restaurant.location.coordinates[1]}</div>
                      <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">Latitude</div>
                    </div>
                    <div className="bg-white dark:bg-gray-900/50 rounded-lg p-4 border border-purple-100 dark:border-gray-700 text-center">
                      <div className="text-2xl font-bold text-purple-600 dark:text-purple-400">{restaurant.location.coordinates[0]}</div>
                      <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">Longitude</div>
                    </div>
                  </div>
                  <p className="text-xs text-gray-500 dark:text-gray-400 text-center">Coordinates for map integration</p>
                </div>
              ) : (
                <div className="text-center py-6">
                  <Navigation size={32} className="mx-auto text-gray-300 mb-3" />
                  <p className="text-gray-400 dark:text-gray-500 font-medium">Location not set</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-gradient-to-r from-gray-50 to-blue-50 dark:from-gray-800 dark:to-gray-800/50 rounded-xl p-4 border border-gray-100 dark:border-gray-700">
            <div className="flex items-center justify-between text-sm">
              ...
              <span className="font-medium text-gray-900 dark:text-gray-100">{new Date(updatedAt).toLocaleDateString('en-US', { 
                year: 'numeric', 
                month: 'short', 
                day: 'numeric',
                hour: '2-digit',
  ...
  <div className="flex flex-col sm:flex-row justify-end gap-3">
    <div className="flex gap-3">
      <Button
        variant="active"
        className={`relative px-5 py-2.5 rounded-xl font-medium transition-all duration-200 overflow-hidden group ${isActive 
